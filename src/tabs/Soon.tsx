...
import { IMovieDetail, getComingSoon, getMovie } from "../api";
...
function Soon(){
  ...
  const onItemClicked =(id:number)=>{ 
    navigate(`/coming-soon/detail/${id}`,{state: {id}})
};
const onOverlayClicked =()=>navigate(`/coming-soon`);
const clickedMovie = soonMovieMatch?.params.id && data?.results.find((e: { id: string; })=>e.id+"" === soonMovieMatch.params.id);
const {id} = useParams();
    ...
    <Body>
     ...
     <AnimatePresence mode="wait">
      ...
      <Overlay onClick={onOverlayClicked} animate={{opacity:1}} exit={{opacity:0}}/>
        <BigMovie layoutId={soonMovieMatch.params.id} >
            {clickedMovie &&
            <>
